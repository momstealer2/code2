...
const updateResultsCount = (count) => {
    if(resultsCount) {
        resultsCount.innerHTML = `Showing <strong>${count}</strong> best sellers`;
    }
}
...
const sortProducts = value => {
    ...
    switch (value) {
        ...
        case "newest":
            cards.sort((a, b) => {
                const aNew = a.querySelector(".bestseller-badge") ? 1 : 0
                const bNew = b.querySelector(".bestseller-badge") ? 1 : 0
                return bNew - aNew
            })
            break
        case "featured":
